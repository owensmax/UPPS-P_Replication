...
UPPS <- read_abcd_tables('abcd_upps01.txt')

UPPS<-UPPS[ which(UPPS$eventname=='2_year_follow_up_y_arm_1'),]
#names(UPPS)[1]<-"src_subject_id"
upps_samp<-merge(UPPS,data_fu2,by="src_subject_id")
NU<-c('upps20_y','upps7_y','upps11_y','upps17_y')
nu_upps<-upps_samp[NU]
alpha(nu_upps)
SS<-c('upps27_y','upps12_y','upps18_y','upps21_y')
ss_upps<-upps_samp[SS]
alpha(ss_upps)
PU<-c('upps37_y','upps39_y','upps36_y','upps35_y')
pu_upps<-upps_samp[PU]
alpha(pu_upps)
LP<-c('upps28_y','upps6_y','upps16_y','upps23_y')
lp_upps<-upps_samp[LP]
alpha(lp_upps)
Lpers<-c('upps22_y','upps19_y','upps24_y','upps15_y')
lpers_upps<-upps_samp[Lpers]
alpha(lpers_upps)

alluppsqs<-cbind(nu_upps,pu_upps,ss_upps,lp_upps,lpers_upps)
alluppsqs<-alluppsqs[complete.cases(alluppsqs), ]

SS_data<-alluppsqs[c(SS)]
NU_data<-alluppsqs[c(NU)]
PU_data<-alluppsqs[c(PU)]
LP_data<-alluppsqs[c(LP)]
Lpers_data<-alluppsqs[c(Lpers)]

SS_cor<-cor(SS_data)
NU_cor<-cor(NU_data)
PU_cor<-cor(PU_data)
LP_cor<-cor(LP_data)
Lpers_cor<-cor(Lpers_data)

SS_omega<-omega(SS_data)
NU_omega<-omega(NU_data)
PU_omega<-omega(PU_data)
LP_omega<-omega(LP_data)
Lpers_omega<-omega(Lpers_data)

omega_tally<-list()
omega_tally['SS']<-SS_omega$omega_h
omega_tally['NU']<-NU_omega$omega_h
omega_tally['PU']<-PU_omega$omega_h
omega_tally['LP']<-LP_omega$omega_h
omega_tally['Lpers']<-Lpers_omega$omega_h

alpha_tally<-list()
alpha_tally['SS']<-SS_omega$alpha
alpha_tally['NU']<-NU_omega$alpha
alpha_tally['PU']<-PU_omega$alpha
alpha_tally['LP']<-LP_omega$alpha
alpha_tally['Lpers']<-Lpers_omega$alpha
